...
static bool submit_upstream_work(CURL *curl, struct work *work)
{
	...
	if (pool->type & POOL_STRATUM) {
		...
		//ZCC Debug
		//applog(LOG_ERR, req);
		//applog(LOG_INFO, "data_str: %s", data_str);
		//applog(LOG_INFO, "work->txs2: %s", work->txs2);
		//
...
// Non-ZCC output mem size
#define BASIC_OPSIZE_MEM 256
...
/*Founder 1*/
static uchar founder_01_output[] = {
...
/*Founder 2*/
static uchar founder_02_output[] = {
...
/*Founder 3*/
static uchar founder_03_output[] = {
...
/*Founder 4*/
static uchar founder_04_output[] = {
...
/*Founder 5*/
static uchar founder_05_output[] = {
...
/*Founder 6*/
static uchar founder_06_output[] = {
...
/*Founder 7*/
static uchar founder_07_output[] = {
...
/* Treasury */
static uchar treasury_output_period_one[] = {
...
static uchar treasury_output_period_two[] = {
...
static uchar treasury_output_period_three[] = {
...
static int period_one_size =
	BASIC_OPSIZE_MEM +
	founder_01_output_len +
	founder_02_output_len +
	founder_03_output_len +
	founder_04_output_len +
	founder_05_output_len +
	founder_06_output_len +
	founder_07_output_len +
	treasury_output_period_one_len;

static int period_two_size =
	BASIC_OPSIZE_MEM +
	treasury_output_period_two_len;

static int period_three_size =
	BASIC_OPSIZE_MEM +
	treasury_output_period_three_len;

static void add_zcc_outputs(uchar *cbtx, int *pcbtx_size, struct work *work) {
	if (work->height < ZCC_FIRST_HALVING) {
		applog(LOG_INFO, "ZCC First period - 9 coinbase outputs");
	...
	else if (work->height < ZCC_SECOND_HALVING) {
		applog(LOG_INFO, "ZCC Second period - 2 coinbase outputs");
	...
	else if (work->height < ZCC_THIRD_HALVING) {
		applog(LOG_INFO, "ZCC Third period - 2 coinbase outputs");
		...
		*pcbtx_size += treasury_output_period_three_len;
		...
		applog(LOG_INFO, "ZCC later periods - 1 coinbase output");
		return;
	}
}
...
static bool gbt_work_decode_full(const json_t *val, struct work *work)
{
	...
	if (tmp) {
		...
		if (opt_algo == ALGO_ZCC) {
			if (work->height < ZCC_FIRST_HALVING)
				cbtx = (uchar*)malloc(BASIC_OPSIZE_MEM + period_one_size);
			else if (work->height < ZCC_SECOND_HALVING)
				cbtx = (uchar*)malloc(BASIC_OPSIZE_MEM + period_two_size);
			else if (work->height < ZCC_THIRD_HALVING)
				cbtx = (uchar*)malloc(BASIC_OPSIZE_MEM + period_three_size);
			else
				cbtx = (uchar*)malloc(BASIC_OPSIZE_MEM);
		}
		else {
			cbtx = (uchar*)malloc(BASIC_OPSIZE_MEM);
		}
		...
		if (opt_algo == ALGO_ZCC) {
			if (work->height < ZCC_FIRST_HALVING)
				cbtx[cbtx_size++] = 9; /* out-counter */
			else if (work->height < ZCC_SECOND_HALVING)
				cbtx[cbtx_size++] = 2; /* out-counter */
			else if (work->height < ZCC_THIRD_HALVING)
				cbtx[cbtx_size++] = 2; /* out-counter */
			else
				cbtx[cbtx_size++] = 1; /* out-counter */
		...
		//->
		if (opt_algo == ALGO_ZCC) {
			add_zcc_outputs(cbtx, &cbtx_size, work);
		}
		//<-
...
int main(int argc, char *argv[])
{
	...
	if (ALGO_ZCC) {
		applog(LOG_INFO, "Using ZCC - hashing Blake256r14");
		allow_getwork = false;
		//. . .
	}
